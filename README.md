...
- Adds a Figma-aligned desktop entrance with 11 selected draggable images and a central click-in link.
- Updates loading image references to match the curated collection.
- Fixes spacing between the Effects heading, Chinese label, and Undo control.
- Reuses noise-lattice calculations without changing texture values; adds an exact numerical regression test.
- Bounds paper texture cache memory to avoid unbounded growth during repeated generation.
- Retains L/M/H quality controls; excludes experimental Light Dots and Focus Melt.
...
- Refines the uploaded-state mobile header, responsive title treatment, fixed result preview, source/mask layout, and bottom action dock across 375px, 390px, and 430px widths.
- Simplifies the mobile FG/BG/RE controls into color status plus foreground/background transparency and mask-reverse actions, while keeping precise color editing on desktop.
- Removes the mobile HUE/SAT/VAL wheel, hex input, and vibration path to keep WAP interaction responsive across mobile browsers and embedded WebViews.
- Keeps upload, camera, presets, randomization, reverse mask, and PNG output synchronized with the desktop processing pipeline.
